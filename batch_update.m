...
%           sigma(t)        3X3
%
function robot = batch_update(robot, H_bar, R_bar, nu_bar)

% Useful constants
dim = size(H_bar, 2);

% Kalman Gain
K = robot.sigma_bar*H_bar'/(H_bar*robot.sigma_bar*H_bar' + R_bar);

% Update the belief
robot.mu    = robot.mu_bar + K*nu_bar;        
% Update the uncertainty
robot.sigma = (eye(dim) - K*H_bar)*robot.sigma_bar;
% Assume symmetry
robot.sigma = (robot.sigma + robot.sigma')/2;
